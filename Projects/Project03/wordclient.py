...
def get_next_word_packet(s):
    ...
    if len(packet_buffer) == 0:
        return None
    byte_length = int.from_bytes(packet_buffer[:2], 'big')
    if len(packet_buffer) >= byte_length+2:
        packet = packet_buffer[:byte_length+2]
        packet_buffer = packet_buffer[byte_length+2:]
        return packet
...
def extract_word(word_packet):
    ...
    byte_length = int.from_bytes(word_packet[2:4])
    return word_packet[2:byte_length].decode()
